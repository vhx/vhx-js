...
class Resource {
  ...
  getToken(cb) {
    let _this = this;
    let path  = `${_this._api.protocol}${window.location.host}/tokens`
  ...
  getParams(client_method, id, options, scope) {
    ...
    if (id && id.match(/me|watching/)) {
      params.url = `${_this._api.protocol}${_this._api.host}/me/watching`;
    } else if (id && id.match(/movies|series/)) {
      params.url = `${_this._api.protocol}${_this._api.host}/`;
    } else {
  ...
  ajaxRequest(args, params) {
    ...
    request[args.http_method](params.url)
      ...
      .query((params.url.match(/movies|series/) ? '' : params.qs))
